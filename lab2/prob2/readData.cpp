...
int getIntBetweenBordersInclusive(const int leftBorder, const int rightBorder, const std::string &message) {
  int value;
  std::cout << message;
  ...
  do {
    ...
    if (value < leftBorder || value > rightBorder) {
      std::cout << "Value should be between " << leftBorder << " and " << rightBorder << " !" << std::endl;
      std::cout << message;
    }
